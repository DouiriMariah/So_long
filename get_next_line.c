...
char	*upgradebuffer(char *myline, char *buffertot)
{
	...
	if (!myline || !buffertot)
		return (NULL);
	newbuffer = malloc((ft_strleni(myline) + 2) * sizeof(char));
	if (!newbuffer)
		return (NULL);
	start_buffer = buffertot;
	if (ft_strleni(buffertot) == 0)
	{
		free(buffertot);
		free(newbuffer);
		return (NULL);
	}
	while (++j <= ft_strleni(buffertot) && myline[j])
...
char	*ft_continu(char *buffer, char *final, int numb, int fd)
{
	...
	if (final)
		line = upgradebuffer(line + ft_strleni(final), line);
	return (final);
}
